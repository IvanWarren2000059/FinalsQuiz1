<template>
  <div class="h-[100vh] bg-gray-100 space-y-4 overflow-auto">
    <div class="w-full">
...
export default {
  ...
  setup() {
    ...
    const editPost = (postId, title, body) => {
      router.push({
        name: "EditPost",
        params: { id: postId, title: title, body: body },
        props: { postId: postId, title: title, body: body },
      });
    };
    ...
    const deletePost = (postId) => {
      axios
        ...
        .then(() => {
          fetchPosts();
          isMyPostsSelected.value = false;
          isNewsFeedSelected.value = true;
          posts.value = posts.value.filter((post) => post.id !== postId);
        })
        .catch((error) => {
